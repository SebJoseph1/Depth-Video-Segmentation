...
def sum_vip_deeplab_losses(outputs, labels):
        ...
        # print(loss)
        semantic_loss = TopKCrossEntropyLoss(num_classes=len(dataloader.classes), ignore_label=255, weight=1.0, top_k_percent_pixels=0.2).forward(labels['semantic'].clone(), outputs['semantic'].clone().float(), labels['semantic_weights'])
        # print(loss)
        center_loss = MSELoss(weight=200.0).forward(labels['center'].clone(), outputs['center'].clone(), labels['semantic_weights'])
        # print(loss)
        offset_loss = L1Loss(weight=1.0).forward(labels['offset'].clone(), outputs['offset'].clone(), labels['semantic_weights'])
        # print(loss)
        nxtoffset_loss = L1Loss(weight=1.0).forward(labels['nxtoffset'].clone(), outputs['nxtoffset'].clone(), labels['semantic_weights'])
        # print(loss)
