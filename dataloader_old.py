...
class SemKittiDataset(Dataset):
    ...
    def __init__(self, dir_input: str, classes: Dict, max_batch_size: int = 32, shuffle: bool = False):
        ...
        for filename in os.listdir(self.dir_input):
        ...
        for s in self.sequences:
            self.batches_count += math.ceil((len(self.sequences[s]) - 1) / self.max_batch_size)
            for item_index in self.sequences[s]:
                if len(self.batches) == 0 or len(self.batches[-1]) >= self.max_batch_size + 1:
                    self.batches.append([])
    ...
    def __getitem__(self, i: int):
        i = self.shuffle_indexes[i]
        sequence = [[], [[], [], []]]
        for item in self.batches[i]:
