...
def mean_squared_error(y_true: torch.Tensor,
                       y_pred: torch.Tensor,
                       force_keep_dims=False) -> torch.Tensor:
...
def sum_vip_deeplab_losses(outputs, labels):
        depth_loss = SILogPlusRelativeSquaredLoss(0.0, 0.1).forward(labels['depth'], outputs['depth'])
        # print(loss)
        semantic_loss = TopKCrossEntropyLoss(num_classes=len(cityscape_dataloader.classes), ignore_label=32, weight=1.0, top_k_percent_pixels=0.2).forward(labels['semantic'], outputs['semantic'].float(), torch.ones_like(labels['semantic_weights']))
        # print(loss)
        center_loss = MSELoss(weight=200.0).forward(labels['center'], outputs['center'], torch.ones_like(labels['semantic_weights']))
        # print(loss)
        offset_loss = L1Loss(weight=0.05).forward(labels['offset'], outputs['offset'], torch.ones_like(labels['semantic_weights']))
        # print(loss)
        nxtoffset_loss = L1Loss(weight=0.05).forward(labels['nxtoffset'], outputs['nxtoffset'], torch.ones_like(labels['semantic_weights']))
        # print(loss)
        loss = depth_loss + semantic_loss + center_loss + offset_loss + nxtoffset_loss
        ...
        log_to_json('all_losses_output', {'depth_loss': torch.mean(depth_loss).item(), 'semantic_loss': torch.mean(semantic_loss).item(), 'center_loss': torch.mean(center_loss).item(), 'offset_loss': torch.mean(offset_loss).item(), 'next_offset_loss': torch.mean(nxtoffset_loss).item()})
        return batch_loss 
