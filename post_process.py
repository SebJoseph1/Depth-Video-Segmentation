...
#     return tf.pad(
#         input_tensor,
#         paddings=[[pad_begin, pad_end], [pad_begin, pad_end], [0, 0]])#doubt
#   else:
#     return tf.pad(
#         input_tensor,
#         paddings=[[0, 0], [pad_begin, pad_end], [pad_begin, pad_end], [0, 0]])#doubt
...
#       [batch, height, width].
#   """
#   return torch.argmax(semantic_logits, dim=1).type(torch.int32)


# def _get_instance_centers_from_heatmap(
#     center_heatmap: torch.Tensor, center_threshold: float, nms_kernel_size: int,
#     keep_k_centers: int) -> Tuple[torch.Tensor, torch.Tensor]:
...
#   center_heatmap = torch.where(
#       torch.greater(center_heatmap, center_threshold), center_heatmap, 0.0)

#   # Non-maximum suppression.
#   padded_map = add_zero_padding(center_heatmap, nms_kernel_size, rank=3)
...
#       padding='valid',
#       pool_mode='max')#doubt
#   pooled_center_heatmap = torch.squeeze(pooled_center_heatmap, axis=0)

#   center_heatmap = torch.where(
#       torch.eq(pooled_center_heatmap, center_heatmap), center_heatmap, 0.0)
#   center_heatmap = torch.squeeze(center_heatmap, axis=2)#doubt

#   # `centers` is of shape (N, 2) with (y, x) order of the second dimension.
#   centers = torch.where(torch.greater(center_heatmap, 0.0))

#   if keep_k_centers > 0 and centers.shape[0] > keep_k_centers:
#     topk_scores, _ = tf.math.top_k(
#         tf.reshape(center_heatmap, [-1]), keep_k_centers, sorted=False)#doubt
#     centers = torch.where(torch.greater(center_heatmap, topk_scores[-1]))

#   return centers, center_heatmap


# def _find_closest_center_per_pixel(centers: torch.Tensor,
#                                    center_offsets: torch.Tensor) -> torch.Tensor:
...
#       center, per pixel.
#   """
#   height = center_offsets.shape[1]
#   width = center_offsets.shape[2]
...
#     semantic_logits: torch.Tensor, center_heatmap: torch.Tensor,
#     center_offsets: torch.Tensor, center_threshold: float,
#     thing_class_ids: torch.Tensor, label_divisor: int, stuff_area_limit: int,
...
#         merge_semantic_and_instance_with_tf_op=merge_semantic_and_instance_with_tf_op,
#         )

#   def call(self, result_dict: Dict[Text, torch.Tensor]) -> Dict[Text, torch.Tensor]:
...
#     """
#     processed_dict = {}
#     (processed_dict['panoptic'],
#      processed_dict['semantic'],
#      processed_dict['instance'],
#      processed_dict['center'],
...
class PostProcessor():
    ...
    def call(self, result_dict: Dict[Text, torch.Tensor]):  # -> Dict[Text, torch.Tensor]:
        ...
        processed_dict = {}
        tensorflow_result = {}
        tensorflow_result['semantic'] = tf.convert_to_tensor(
            result_dict['semantic'].detach().numpy().transpose(0, 2, 3, 1))
        tensorflow_result['center'] = tf.convert_to_tensor(result_dict['center'].detach().numpy().transpose(0, 2, 3, 1))
        tensorflow_result['offset'] = tf.convert_to_tensor(result_dict['offset'].detach().numpy().transpose(0, 2, 3, 1))
        ...
        (processed_dict['panoptic'],
         ...
         processed_dict['instance_scores']
            ...
            tensorflow_result['semantic'],
            tensorflow_result['center'],
            tensorflow_result['offset'])

        torch_processed_dict = {}
        torch_processed_dict['panoptic'] = torch.from_numpy(processed_dict['panoptic'].numpy())
        torch_processed_dict['semantic'] = torch.from_numpy(processed_dict['semantic'].numpy())
        torch_processed_dict['instance'] = torch.from_numpy(processed_dict['instance'].numpy())
        torch_processed_dict['center'] = torch.from_numpy(processed_dict['center'].numpy())
        torch_processed_dict['instance_scores'] = torch.from_numpy(processed_dict['instance_scores'].numpy())
        return torch_processed_dict
