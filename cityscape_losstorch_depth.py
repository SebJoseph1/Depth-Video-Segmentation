...
def sum_vip_deeplab_losses(images, outputs, labels):
        ...
        semantic_loss = TopKCrossEntropyLoss(num_classes=len(cityscape_dataloader.classes), ignore_label=32, weight=1.0, top_k_percent_pixels=0.2).forward(labels['semantic'], outputs['semantic'].float(), labels['semantic_weights'])
        # print(loss)
        center_loss = MSELoss(weight=200.0).forward(labels['center'], outputs['center'], labels['semantic_weights'])
        # print(loss)
        offset_loss = L1Loss(weight=0.05).forward(labels['offset'], outputs['offset'], labels['semantic_weights'])
        # print(loss)
        nxtoffset_loss = L1Loss(weight=0.05).forward(labels['nxtoffset'], outputs['nxtoffset'], labels['semantic_weights'])
        # print(loss)
